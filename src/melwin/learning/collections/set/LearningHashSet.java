...
public class LearningHashSet {
	...
	private static class Element {
		...
		/*
		 * The contract between equals() and hashCode() is: - If two objects are equal,
		 * then they must have the same hash code. - If two objects have the same hash
		 * code, they may or may not be equal.
		 * 
		 * NOTE : HashSet is backed by HashMap, so the Keys are the Elements and values
		 * are all references to the SAME dummy objects.
		 */

		/*
		 * Default equals() public boolean equals(Object obj) { // checks if obj are
	...
	public static void main(String[] args) {
		...
		if (ts.add(new Element(1)))
			System.out.println("new Element(1) added");
		// hashCode = 1, goes to bucket-1

		System.out.println("\nGoing to add : new Element(11)");
		if (ts.add(new Element(11)))
			System.out.println("new Element(11) added");
		// hashCode = 1, goes to bucket-1
		// Since the objects the different insertion is successful

		System.out.println("\nGoing to add : new Element(2)");
		if (ts.add(new Element(2)))
			System.out.println("new Element(2) added");
		...
		while (i.hasNext()) {
			...
			if (current.roll_no == 11)
				current.roll_no = 0;
			// don't do this !!!
			// better remove the element and reinsert !!!
		}

		if (ts.contains(new Element(0)))
